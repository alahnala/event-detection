...
class PhraseMining(object):
	...
	def __init__(self, file_name, min_support=10, max_phrase_size=40, alpha=4,stopwords="topmine-master/topmine_src/stopwords/allie.txt"):
	...
	def _preprocess_input(self, filename, stopwords):
		...
		num_docs = 0
		for line in f:
			# if "follow" in line.lower() and "please" in line.lower():
			#	 print("Check line", line.lower(), i)
			line = [token.lower().strip() for token in line.split() if not re.match('#.+', token) and not re.match('@.+', token)]
			...
			# print(line)
			# print(sentences_no_punc)
			sentences_no_punc = line
			# print(sentences_no_punc)
			# print("len(sentences_no_punc)", len(sentences_no_punc))
		...
		for doc in documents:
			# Allie mod. Already removed stopwords.
			documents2.append(' '.join([word for word in doc.split() if word not in stopwords]))
			# documents2.append(' '.join([word for word in doc.split()]))
