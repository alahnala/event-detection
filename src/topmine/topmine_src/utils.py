def store_partitioned_docs(partitioned_docs, path="topmine/intermediate_output/partitioneddocs.txt"):
    f = open(path, 'w')
    for document in partitioned_docs:
        f.write(", ".join(" ".join(str(word) for word in phrase) for phrase in document))
        f.write("\n")

def load_partitioned_docs(path="topmine/intermediate_output/partitioneddocs.txt"):
    f = open(path, 'r')
    partitioned_docs = []
    document_index = 0
    for line in f:
...
def store_vocab(index_vocab, path="topmine/intermediate_output/vocab.txt"):
    """
...
def load_vocab(path="topmine/intermediate_output/vocab.txt"):
    """
...
def store_frequent_phrases(frequent_phrases, path='topmine/output/frequent_phrases.txt'):
    f = open(path, 'w')
    for phrase, val in enumerate(frequent_phrases):
        f.write(str.format("{0} {1}\n",phrase, val))
    f.close()

def store_phrase_topics(document_phrase_topics, path="topmine/intermediate_output/phrase_topics.txt"):
    """
...
def store_most_frequent_topics(most_frequent_topics, prefix_path="topmine/output/topic"):
