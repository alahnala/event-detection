...
def ToPMine_algorithm(file_name, folder_name, min_support=40, ngram_limit=5, num_topics=4, alpha=2):
	...
	'''
	phrase_mining_cmd = "python src/topmine/topmine_src/run_phrase_mining.py {} {} {} {} {}".format(file_name, ngram_limit, min_support, folder_name, alpha)
	get_output_of(phrase_mining_cmd)

	#Update, we only need the most frequent phrases which is found in the first program.
	# phrase_lda_cmd = "python3 topmine/topmine_src/run_phrase_lda.py {} {} {}".format(num_topics, file_name, folder_name)
	# print(get_output_of(phrase_lda_cmd))
	return get_output_of(phrase_mining_cmd)

def ToPMine_algorithm_2(file_name, folder_name, min_support=40, ngram_limit=5, num_topics=4, alpha=2, stopword_file="src/topmine/topmine_src/stopwords.txt"):

	phrase_mining_cmd = "python src/topmine/topmine_src/run_phrase_mining.py {} {} {} {} {} {}".format(file_name, ngram_limit, min_support, folder_name, alpha, stopword_file)
...
def main():
	...
	os.makedirs("src/topmine/"+folder_name + "intermediate_output/", exist_ok=True)
	os.makedirs("src/topmine/"+folder_name + "output/", exist_ok=True)
	# quit()
